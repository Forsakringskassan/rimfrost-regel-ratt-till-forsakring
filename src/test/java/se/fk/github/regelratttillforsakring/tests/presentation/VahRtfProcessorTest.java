...
public class VahRtfProcessorTest extends KafkaTestBase
{
   ...
   @Test
   void testReceiveVahRtfRequest_withUnemployedPnr()
   {
      UUID processId = UUID.randomUUID();
      String personnummer = "12345678-9002";
      var request = ImmutableVahRtfRequest.builder()
            .processId(processId)
            .pnr(personnummer)
            .build();
      ...
      await()
            ...
            .untilAsserted(() -> {
               ...
               VahRtfResponse response = inMemorySink.received().getFirst().getPayload();
               assertThat(response.processId()).isEqualTo(processId);
               assertThat(response.hasArbetsgivare()).isFalse();
               assertThat(response.isBokford()).isTrue();
            });
   }

   @Test
   void testReceiveVahRtfRequest_withBadPnr()
   {
      UUID processId = UUID.randomUUID();
      ...
      await()
            ...
            .untilAsserted(() -> {
               ...
               assertThat(response.processId()).isEqualTo(processId);
               assertThat(response.hasArbetsgivare()).isFalse();
               assertThat(response.isBokford()).isFalse();
            });
   }
